...
public class UserActivity {
    ...
    public void seeAllQuestions(UserActivity userActivity, User user) throws NumberFormatException, IOException {
        ...
        if (questions.size() == 0) {
            System.out.println("no questions are not posted ");
        } else {
            for (Question question : questions) {
                System.out.println(question.getId() + "Question" + question.getQuestionBody());
            }
            QnAapp.questionMenu(userActivity, user);
        }
    ...
    public void deleteQuestion(UserActivity userActivity, User user) throws NumberFormatException {
        System.out.println("enter answer you need to delet");
        Question question = getQuestion();
        if (user.getUserRole() == UserRole.ADMIN) {
            ...
            if (question.getUser() == user) {
                ...
                System.out.println("you are not deleting this question");

            }
        }
        if (QuestionServiceImpl.questions.size() == 0)
            QnAapp.menu(user,userActivity);
